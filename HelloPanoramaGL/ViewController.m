...
@implementation ViewController//视图控制
...
-(void)selectPanorama:(NSInteger)index
{
    ...
    if(index == 0)
    {
        ...
        [(PLSpherical2Panorama *)panorama setImage:[PLImage imageWithPath:[[NSBundle mainBundle] pathForResource:@"pano_sphere1" ofType:@"jpg"]]];
    }
    ...
    //Add a hotspot
    PLTexture *hotspotTexture = [PLTexture textureWithImage:[PLImage imageWithPath:[[NSBundle mainBundle] pathForResource:@"hotspot" ofType:@"png"]]];//读取热点图片;
    
    PLHotspot *hotspot = [PLHotspot hotspotWithId:(1) texture:hotspotTexture atv:0.0f ath:-10.0f width:0.05f height:0.05f];
    //产生一个随机数作为id，纹理的位置：atv：垂直方向（－90,90）ath：水平方向：（－180，180），width：宽，height：高
...
-(void)view:(UIView<PLIView> *)pView didClickHotspot:(PLHotspot *)hotspot screenPoint:(CGPoint)point scene3DPoint:(PLPosition)position
{
    ...
    PLHotspot *hotspot1 = [PLHotspot hotspotWithId:(kIdMin + random() % ((kIdMax + 1) - kIdMin)) texture:hotspotTexture atv:0.0f ath:-8.0f width:0.08f height:0.08f];
